...
def languages():
    ...
    dist_matrix = load_distance_matrix(fname)

    tree = linkage(y = dist_matrix[0])
    plt.figure(figsize=(16, 8))
    D = dendrogram(tree, labels = dist_matrix[1], orientation = 'left')
    plt.show()

def main() -> int:
    chosen_dataset = int(sys.argv[1]) #we choose subroutine via command line argument
